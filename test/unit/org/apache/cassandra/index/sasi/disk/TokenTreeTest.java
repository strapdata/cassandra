...
public class TokenTreeTest
{
    ...
    public void testMergingOfEqualTokenTrees(SortedMap<Long, LongSet> tokensMap) throws Exception
    {
        ...
        for (Map.Entry<Long, LongSet> entry : tokensMap.entrySet())
        {
            ...
            LongSet found = result.getOffsets();
            Assert.assertTrue(sameKeys(entry.getValue(), found));
        }
    }

    /**
     * Compare two com.carrotsearch.hppc.LongSet by the value of their keys.
     * @param a
     * @param b
     * @return true if the keys are identicals
     */
    private static boolean sameKeys(LongSet a, LongSet b) {
        if (a == b) {
            return true;
        }
        ...
        long[] aArray = a.toArray(), bArray = b.toArray();
        Arrays.sort(aArray);
        Arrays.sort(bArray);
